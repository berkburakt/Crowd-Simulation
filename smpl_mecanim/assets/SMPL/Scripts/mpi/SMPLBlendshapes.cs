...
public class SMPLBlendshapes : MonoBehaviour {
    ...
    public void updateCollider()
    {
        ...
        //transform.parent.GetComponent<PlayerWalk>().anim.speed = 0;

        var collider = transform.parent.GetComponent<CapsuleCollider>();
        var mesh = GetComponent<SkinnedMeshRenderer>();
        /*var foot = transform.parent.GetChild(1).GetChild(0).GetChild(0).GetChild(0).GetChild(0).GetChild(0);
        var head = transform.parent.GetChild(1).GetChild(0).GetChild(2).GetChild(0).GetChild(0).GetChild(1).GetChild(0);
        float height;
        Debug.Log(foot.name);
        if(foot.name == "m_avg_L_Foot")
        {
            height = (head.transform.position.y - foot.transform.position.y)+0.12f;
        } else
        {
            height = (head.transform.position.y - foot.transform.position.y)+0.1f;
        }*/
        float heightMultiplier;
        
        if (transform.name == "f_avg")
        {
            heightMultiplier = (mesh.GetBlendShapeWeight(0) - mesh.GetBlendShapeWeight(1)) * 0.005f;
        } else
        {
            heightMultiplier = (mesh.GetBlendShapeWeight(1) - mesh.GetBlendShapeWeight(0)) * 0.005f; ;
        }
        Debug.Log(heightMultiplier);
        collider.height += heightMultiplier;
        collider.center = new Vector3(GetComponent<SkinnedMeshRenderer>().sharedMesh.bounds.center.x, collider.height/2, GetComponent<SkinnedMeshRenderer>().sharedMesh.bounds.center.z);
        //collider.transform.position = new Vector3(collider.transform.position.x, 0, collider.transform.position.z);

    }

    public void InitShapeParms()
    {
        for (int bi = 1; bi < _numShapeParms; bi++)
            _shapeParms[bi] = Random.Range(-2.0f, 2.0f);
        ...
        float heightMultiplier = _shapeParms[1];
